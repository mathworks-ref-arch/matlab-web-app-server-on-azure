...
| **Assign Public IP Address to VM Hosting MATLAB Web App Server** | Specify whether to assign a public IP address to the VM hosting the server and enable "Public network access" for the storage account. <ul><li>If you select "Yes", the storage account's public network access is enabled so that you can access it from the virtual network, including any VMs in the virtual network. This setting allows access to the storage account using the MATLAB Web App Server VM and from "Selected virtual networks and IP addresses", as specified in the Azure Portal. Additionally, the first entry in both the **IP Addresses Permitted to Remote into Server VM in CIDR Notation** and **IP Addresses Allowed to Access MATLAB Web App Server Apps Home Page in CIDR Notation** parameters are added under "Add IP ranges to allow access from the internet or your on-premises networks" in the Azure Portal under Networking.</li><li>If you select "No", the storage account's public network access is disabled. You must create a new virtual machine and add it to the same virtual network as the MATLAB Web App Server deployment. The ability to access the web apps home page or remotely connect to the server machine can be accomplished only through this virtual machine.</ul></li> <p><p>**Note:** If are using an existing virtual network, then you must manually add a private or service endpoint to the virtual network. For details, see [Create Endpoint in Virtual Network](#create-endpoint-in-virtual-network).</p> |
